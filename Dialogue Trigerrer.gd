...
func on_Area2D_body_entered(body):
	if body is Player and not self.name in GlobalVariables.triggered_dialouge:
		...
		if not display_cutscene:
			...
			if not GlobalVariables.displayed_cutscene:
				GlobalVariables.displayed_cutscene = true
		...
		if is_end:
			GlobalVariables.get_node("Theme2").play()
			...
			for t in credit_text:
				...
				yield(get_tree().create_timer(3), "timeout")
			GlobalVariables.get_node("Tween").interpolate_property(GlobalVariables.get_node("Theme2"),"volume_db",0,-80,5,Tween.TRANS_LINEAR,Tween.EASE_IN_OUT)
			GlobalVariables.get_node("Tween").start()
